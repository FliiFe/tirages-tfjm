  ...
  <div id="tirage">
    ...
    >
      <template slot="items" slot-scope="props">
        <td>
          <span
            :class="$store.state.tirages[poule].team === props.item.name ? 'green--text text--accent-2' : ''"
        ...
        <td v-for="p in $store.state.problemes" :key="p">
          <v-icon
            ...
            class="checkbox"
          >{{props.item['p' + p] === 1 ? 'done': props.item['p' + p] === -1 ? 'close' : props.item['p' + p] === -2 ? 'radio_button_checked' : 'radio_button_unchecked'}}</v-icon>
        </td>
      </template>
    </v-data-table>
    <!-- Si c'est à l'équipe de tirer -->
    <div
      v-if="!spectate && $store.state.tirages[poule].team === $store.state.trigram && !dialog && !hasPendingProblem"
    >
    ...
    <div v-else-if="$store.state.tirages[poule].team.length >= 1">
      <em>
        L'équipe
        <b>{{ $store.state.tirages[poule].team }}</b> tire un problème
      </em>
    </div>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <div v-if="$store.state.problemes - 5 - refus.length >= 0">
            Il vous reste
            <span class="red--text">{{$store.state.problemes - 5 - refus.length}} refus</span>
          </div>
          <div v-else>
            <span
              class="red--text"
            >Vous avez {{-$store.state.problemes + 5 + refus.length}} pénalité{{-$store.state.problemes + 5 + refus.length == 1 ? '' : 's'}}</span>
          </div>
          <!-- TODO: meme pb -->
        </v-card-text>
        <v-card-actions>
...
export default {
    ...
    computed: {
        ...
        refus() {
            ...
            return this.$store.state.trigram
                ? this.$store.state.tirages[this.poule].pb.find(
                      t => t.name === this.$store.state.trigram
                  ).refused
                : []
        }
    },
